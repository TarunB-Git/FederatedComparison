#!/usr/bin/env python3
"""FedProx for hierarchical GRU on SC2EGSet.

Same as FedAvg but with a proximal term μ/2·||w - w_global||² added to each
client's local loss to penalise drift from the global model.
"""
...
def client_update_fedprox(
    *,
    ...
    """Train local model with proximal regularisation towards global model."""
    model = HierGRU(**model_cfg).to(device)
    model.load_state_dict(global_state)
    use_exact_head = model.use_exact_head

    # Reference parameters for proximal term
    ...
    for _ in range(local_epochs):
        for bidx, (x, y_coarse, y_fine, y_exact, lengths) in enumerate(loader):
            ...
            task_loss = coarse_loss_weight * coarse_loss + fine_loss_weight * fine_loss
            if exact_logits is not None and exact_loss_weight > 0:
                task_loss = task_loss + exact_loss_weight * F.cross_entropy(exact_logits, y_exact)

            # Proximal term: μ/2 * Σ||w_i - w_global_i||²
            prox = torch.tensor(0.0, device=device)
            for p, p0 in zip(model.parameters(), ref_params):
                prox = prox + torch.sum((p - p0) ** 2)
            loss = task_loss + 0.5 * mu * prox
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="FedProx hierarchical GRU on SC2EGSet.")
    ap.add_argument("--dataset-dir", required=True)
    ap.add_argument("--outdir", default="runs/hier_fedprox")
    ap.add_argument("--cross-run-csv", default="runs/cross_run_results.csv")

    ap.add_argument("--mu", type=float, default=0.01, help="Proximal strength.")
...
def main() -> None:
    ...
    print(f"[fedprox] {len(eligible_clients)} eligible clients, mu={args.mu}", flush=True)
    ...
    config.update({
        "mode": "fedprox", "architecture": args.model_name, "race": args.race, "n_features": len(feature_cols),
        **action_context_meta,
    ...
    for rnd in range(start_round, args.rounds + 1):
        ...
        for cid in selected:
            ...
            client_t0 = time.time()
            c_state, c_n, c_loss = client_update_fedprox(
    ...
    test_json["mode"] = "fedprox"
    test_json["mu"] = args.mu
    ...
    plot_confusion_matrix(test_m["_cm_coarse"], coarse_names, outdir / "test_coarse_confusion.png", "Coarse Action Confusion (FedProx)")
    ...
    # per-client
    from hier_fedavg import _save_per_client_test
    ...
    append_cross_run_row(
        cross_csv, mode="fedprox", dataset_dir=args.dataset_dir,
    ...
    print(f"[done] fedprox mu={args.mu}. best_round={best_round}")
    print(
        "  exact_top1="
